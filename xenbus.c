...
static unsigned long total_credit = 625000UL;
/* mlr-end */
...
static void xen_read_bandwidth(struct xenbus_device *dev)
{
	...
	//read bandwidth from config
	bandwidthstr = xenbus_read(XBT_NIL, dev->nodename, "bandwidth", NULL);
	if (IS_ERR(bandwidthstr))
		return;
	//convert bandwidth to credit
	s = bandwidthstr;
...
static int domain_number()
{
	...
	int count = 0;
	pd = &dev_list_node;
	for (pd = &ddev_list_node; *pd != NULL; pd = &(*pd)->next_in_list)
		count++;
	return count;
...
{
	char *averagestr;
	averagestr = xenbus_read(XBT_NIL, dev->nodename, "average", NULL);
	if (IS_ERR(averagestr))
		return false;
	char *averageset = "true";
	if (_stricmp(averagestr, averageset) == 0)
		return true;
	else
		return false;
}
...
/* mlr-begin : changed, add para unsigned long *initial */
static void xen_net_read_rate(struct xenbus_device *dev,
			      unsigned long *bytes, unsigned long *usec, unsigned long *initial)
/* mlr-end */
{
	char *s, *e;
	unsigned long b, u;
